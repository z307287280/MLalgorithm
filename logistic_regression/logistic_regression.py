...
class LogisticRegression:
    def __init__(self, lr=0.001, max_iter=10000, keep_bias=True, shuffle=True, multi_class='auto'):
        self.lr = lr
    ...
    def initialize_coef(self, shape):
        if self.multi_class == 'binary':
            self.coef['weights'] = np.zeros(shape[0])
        else:
            self.coef['weights'] = np.zeros(shape)
